...
--TASK 3
select  first_name as first_name, len(first_name) as name_length
from employees
...
sum(case when datediff(year, hire_date, getdate()) > 3 then 1 else 0 end) * 1.0 / count(*) * 100 as percent_above_3_years
from employees
group by department_id
--TASK 6
select jobdescription, min(spacemanid) as least_experienced_id, max(spacemanid) as most_experienced_id
from (select jobdescription, spacemanid, rank() over(partition by jobdescription order by mKssioncount) as min_rank,
rank() over(partition by jobdescription order by missioncount desc) as max_rank
from personal) ranked
...
(select string_agg(value, '') from string_split(@str, '') where value collate latin1_general_bin like '[A-Z]') as uppercase_letters,
(select string_agg(value, '') from string_split(@str, '') where value collate latin1_general_bin like '[a-z]') as lowercase_letters,
(select string_agg(value, '') from string_split(@str, '') where value like '[0-9]') as digits,
(select string_agg(value, '') from string_split(@str, '') where value collate latin1_general_bin not like '[A-Za-z0-9]') as other_chars
--TASK 2
...
--TASK 3
update equations
set totalsum = case when isnumeric(equation) = 1 then cast(equation as int)
else (select sum(cast(value as int))
