...
--TASK 1
select id, parsename(replace(name, ',', '.'), 2) as name, parsename(replace(name, ',', '.'), 1) as surname
from testmultiplecolumns
--TASK 2
select * from testpercent
where strs like '%[%]%'
--TASK 3
select id, parsename(replace(vals, '.', '.'), 3) as part1, parsename(replace(vals, '.', '.'), 2) as part2,
parsename(replace(vals, '.', '.'), 1) as part3
from splitter
--TASK 4
declare @str varchar(100) = '1234abc123456xyz1234567890ads'
select translate(@str, '0123456789', 'xxxxxxxxxx') as replaced
--TASK 5
select * from testdots
where len(vals) - len(replace(vals, '.', '')) > 2
--TASK 6
select texts, len(texts) - len(replace(texts, ' ', '')) as space_count
from countspaces
--TASK 7
...
select left(@val, patindex('%[0-9]%', @val) - 1) as chars,
substring(@val, patindex('%[0-9]%', @val), len(@val)) as numbers
--TASK 2
...
where ordinal = 3
--TASK 5
select w.area, w.financialweek, w.dayname,
cast(isnull(w.saleslocal, 0) + isnull(w.salesremote, 0) as float) 
/ t.total_sales * 100 as percentage_sales
from weekpercentagepuzzle w
join (select area, financialweek, sum(isnull(saleslocal, 0) + isnull(salesremote, 0)) as total_sales
from weekpercentagepuzzle
group by area, financialweek) t
on w.area = t.area and w.financialweek = t.financialweek
--TASK 6
select p1.id, 
...
--TASK 7
select vals,
left(vals, patindex('%[^0-9]%', vals + 'a') - 1) as starting_integer
from getintegers
where vals like '[0-9]%'
--DIFFICULT
--TASK 1
select id,
stuff(stuff(vals, 1, charindex(',', vals), 
substring(vals, charindex(',', vals) + 1, charindex(',', vals) - 1)), 
charindex(',', vals) + 1, charindex(',', vals) - 1, 
left(vals, charindex(',', vals) - 1)) as swapped
from multiplevals
--TASK 2
...
--TASK 3
with totals as (
select area, financialweek, sum(isnull(saleslocal,0) + isnull(salesremote,0)) as total_sales
from weekpercentagepuzzle
group by area, financialweek)
select w.area, w.financialweek, w.dayname,
cast(isnull(w.saleslocal,0) + isnull(w.salesremote,0) as float) / t.total_sales * 100 as percentage_sales
from weekpercentagepuzzle w
join totals t on w.area = t.area and w.financialweek = t.financialweek
